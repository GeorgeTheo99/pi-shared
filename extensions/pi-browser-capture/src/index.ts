...
export default function (pi: ExtensionAPI) {
  ...
  // Legacy browse_* aliases. Prefer browser_* in new prompts and documentation.
  pi.registerTool({
    name: "browse_open_url",
    label: "Browse Open URL",
    description: "Legacy alias for browser_navigate. Prefer browser_navigate.",
    promptSnippet: "browse_open_url legacy alias for browser_navigate",
    parameters: Type.Object({ url: Type.String({ description: "Absolute public URL to open" }) }),
    async execute(_id, params, signal) {
      abortIfNeeded(signal);
      return textResult(await runtime.run(() => runtime.navigate(params.url)));
    },
  });

  pi.registerTool({
    name: "browse_open_tab",
    label: "Browse Open Tab",
    description: "Legacy alias for browser_open_tab. Prefer browser_open_tab.",
    parameters: Type.Object({ url: Type.Optional(Type.String({ description: "Optional absolute public URL to open" })) }),
    async execute(_id, params, signal) {
      abortIfNeeded(signal);
      return textResult(await runtime.run(() => runtime.newTab(params.url)));
    },
  });

  pi.registerTool({
    name: "browse_list_tabs",
    label: "Browse List Tabs",
    description: "Legacy alias for browser_list_tabs. Prefer browser_list_tabs.",
    parameters: Type.Object({}),
    async execute(_id, _params, signal) {
      abortIfNeeded(signal);
      const tabs = await runtime.run(() => runtime.listTabs());
  ...
  pi.registerTool({
    ...
    description: "Legacy alias for browser_switch_tab. Prefer browser_switch_tab.",
    parameters: Type.Object({ tab_index: Type.Number({ description: "Tab index to switch to" }) }),
    async execute(_id, params, signal) {
      abortIfNeeded(signal);
      return textResult(await runtime.run(() => runtime.setActiveTab(params.tab_index)));
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Legacy alias for browser_close_tab. Prefer browser_close_tab.",
    parameters: Type.Object({ tab_index: Type.Optional(Type.Number({ description: "Tab index to close" })) }),
    async execute(_id, params, signal) {
      abortIfNeeded(signal);
      return textResult(await runtime.run(() => runtime.closeTab(params.tab_index)));
    },
  });
  ...
  pi.registerTool({
    name: "browse_click",
    label: "Browse Click",
    description: "Legacy alias for browser_click. Prefer browser_click.",
    parameters: Type.Object({ selector: Type.String({ description: "Playwright CSS selector" }) }),
    async execute(_id, params, signal) {
      abortIfNeeded(signal);
      return textResult(await runtime.run(() => runtime.click(params.selector)));
    },
  });

  pi.registerTool({
    name: "browse_type_text",
    label: "Browse Type Text",
    description: "Legacy alias for browser_type. Prefer browser_type.",
    parameters: Type.Object({
      selector: Type.String({ description: "Playwright CSS selector" }),
      text: Type.String({ description: "Text to type" }),
    ...
    async execute(_id, params, signal) {
      abortIfNeeded(signal);
      return textResult(await runtime.run(() => runtime.typeText(params.selector, params.text, params.clear ?? true, params.submit ?? false)));
    },
  });

  pi.registerTool({
    name: "browse_wait_for",
    label: "Browse Wait For",
    description: "Legacy alias for browser_wait_for. Prefer browser_wait_for.",
    parameters: Type.Object({
      selector: Type.Optional(Type.String({ description: "CSS selector to wait for" })),
      url_contains: Type.Optional(Type.String({ description: "URL fragment to wait for" })),
      state: Type.Optional(Type.String({ description: "Element state. Defaults to visible." })),
      timeout_ms: Type.Optional(Type.Number({ description: "Timeout in ms. Defaults to 15000." })),
    }),
    async execute(_id, params, signal) {
      abortIfNeeded(signal);
      return textResult(await runtime.run(() => runtime.waitFor(params.selector, params.url_contains, (params.state as WaitState | undefined) ?? "visible", params.timeout_ms ?? config.defaultTimeoutMs)));
    },
  });

  pi.registerTool({
    name: "browse_extract_text",
    label: "Browse Extract Text",
    description: "Legacy alias for browser_extract_text. Prefer browser_extract_text.",
    parameters: Type.Object({
      selector: Type.Optional(Type.String({ description: "CSS selector, defaults to page body" })),
      max_chars: Type.Optional(Type.Number({ description: "Max characters. Defaults to 8000." })),
    }),
    async execute(_id, params, signal) {
      abortIfNeeded(signal);
      return truncatedTextResult(await runtime.run(() => runtime.extractText(params.selector, params.max_chars ?? 8_000)));
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Legacy alias for browser_screenshot. Prefer browser_screenshot.",
    parameters: Type.Object({
      label: Type.Optional(Type.String({ description: "Label for the screenshot filename" })),
      full_page: Type.Optional(Type.Boolean({ description: "Capture full page. Defaults to true." })),
    }),
    async execute(_id, params, signal, _onUpdate, ctx) {
      abortIfNeeded(signal);
      return textResult(await runtime.run(() => runtime.screenshot(undefined, params.label, params.full_page ?? true, ctx.cwd)));
    },
  });

  pi.registerTool({
    name: "browse_console_logs",
    label: "Browse Console Logs",
    description: "Legacy alias for browser_console_logs. Prefer browser_console_logs.",
    parameters: Type.Object({
      limit: Type.Optional(Type.Number({ description: "Max entries. Defaults to 50." })),
      level: Type.Optional(Type.String({ description: "Filter by level." })),
    }),
    async execute(_id, params, signal) {
      abortIfNeeded(signal);
      const logs = runtime.getConsoleLogs(params.limit ?? 50, params.level);
  ...
  pi.registerTool({
    name: "browse_page_state",
    label: "Browse Page State",
    description: "Legacy alias for browser_page_state. Prefer browser_page_state.",
    parameters: Type.Object({}),
    async execute(_id, _params, signal) {
      abortIfNeeded(signal);
      return textResult(await runtime.run(() => runtime.pageState()));
    },
  });
}
