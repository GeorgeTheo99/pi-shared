...
3. The selected model generates a concise, self-contained continuation prompt from a best-effort-redacted transcript, optional focus, active goal, and work plan. Hidden reasoning, raw tool arguments, and shell output marked `excludeFromContext` are excluded. On Pi `0.87.1`, the transcript uses the canonical session projection, honoring branch-local context omissions and replacements; Pi `0.85.1` retains the compaction-aware entry conversion. Heuristic redaction cannot guarantee removal of every secret, so do not hand off a transcript known to contain sensitive values.
4. Pi inspects unexpired, unanswered outgoing and incoming peer response requests (including already-surfaced requests and outgoing transcript records whose status-history entries were evicted). A fixed warning appears before review and in the child kickoff/orientation: peer requests and replies stay bound to the exact parent session, never transfer/forward to a child or fork, and never wake a peer. Inspection failures are reported as unavailable, not zero requests. Pi opens the continuation and warning in an editor. Review/edit it, then submit to proceed or cancel to keep the parent session. If the peer-request snapshot changes during review, replacement is cancelled so a new attempt can obtain fresh consent. This is an advisory snapshot, not an atomic transfer: messages may still arrive after the final check and remain bound to the parent.
