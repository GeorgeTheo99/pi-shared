...
GENERATED_HEADER = "# Generated by pi-shared/bin/pi-catalog — do not hand-edit."
BOOTSTRAP_MARKER = "# Pi catalog state: unconfigured (management-only)."
...
def render_launchers(
    ...
    *,
    ...
    lines = [
        GENERATED_HEADER,
        *([BOOTSTRAP_MARKER] if allow_empty_catalog and not aliases else []),
        "# Source from ~/.zshrc. Regenerated from model-aliases.json (the",
        "# model-gateway public catalog contract). No claude-*/codex-* — pi-* only.",
        "",
        "# Module-owned operator CLIs live here; preserve existing PATH precedence.",
    ...
    if not rows:
        lines += [
            '  echo ""',
            '  echo "No gateway model launchers configured yet."',
            '  echo "Use pi, then /login and /model for direct providers."',
        ]
        if aliases_path:
            lines += [f"  print -r -- {shlex.quote('Configure the gateway alias export at ' + aliases_path + ', then run pi-regen.')}"]
    if direct_launchers:
        lines += [
            '  echo ""',
            '  echo "Direct Pi:"',
...
def _render_pi_regen(*, aliases_path, models_out, launchers_out, provider_name, gateway_url, gateway_api_key, pi_agent_dir, direct_launchers, shared_dir, omlx_status_path, omlx_status_url, allow_empty_catalog=False) -> list[str]:
    ...
    return [
        ...
        f"  {cmd_str} \"$@\" || return $?",
        *([f"  zsh -n {shlex.quote(launchers_out)} || return $?",
           f"  source {shlex.quote(launchers_out)}"] if launchers_out else []),
        "}",
        "",
    ]
...
def _render_pi_restart(*, gateway_url: str, auto_regen: bool = False, aliases_path: str | None = None) -> list[str]:
    ...
    out = [
        ...
        '    echo "  omlx      oMLX inference server (server-ci, or oMLX app/Homebrew CLI)"',
        ...
        "    return $?",
        "  fi",
        '  if [ "$svc" = "omlx" ] && ! command -v server-ci >/dev/null 2>&1 && command -v omlx >/dev/null 2>&1; then',
        "    # oMLX owns app/Homebrew service detection; never install or adopt a service here.",
        "    omlx restart",
        "    return $?",
        "  fi",
        "  local rc=0",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not isinstance(aliases, dict):
            raise ValueError("expected an alias catalog object")
        if not aliases:
            ...
            for existing in (args.launchers_out, args.legacy_launchers):
                if existing and (existing.exists() or existing.is_symlink()):
                    ...
                    if not previous.startswith(GENERATED_HEADER + "\n" + BOOTSTRAP_MARKER + "\n"):
    ...
    if not args.quiet:
        if not aliases:
            print("pi-catalog: unconfigured — management launchers ready; models.json unchanged")
        else:
