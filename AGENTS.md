...
- Delegate when appropriate: use `spawn_subagent` when you would need 5+ sequential read/grep/find calls, when the task has independent subtasks, or when a specialist perspective (review, planning, isolated implementation) would improve the result. Use `deep_research` instead of 3+ sequential `web_search`+`web_fetch` calls. Do not delegate tiny questions or quick local checks. If git worktree/branch isolation is needed, create/manage that explicitly before delegating.
