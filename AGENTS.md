...
- **Specialist subagents for data-intensive queries:** When querying structured internal data sources (databases, CRMs, analytics warehouses), prefer delegating to a specialist subagent that has schema knowledge (e.g., `field-data-analyst` if available via scope `user` or `all`). For multi-entity or multi-source queries, use parallel mode to spawn multiple specialist instances concurrently. Do not attempt complex structured queries from the main agent without first loading the relevant skills — they contain critical schema documentation in their resource files.
