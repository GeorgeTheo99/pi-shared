# Advanced and source-install reference

For the recommended Homebrew install, start with the [main README](../README.md).
This reference covers shared-resource internals, source-only installation,
and legacy generated shell launchers. Homebrew uses the unified `pi` CLI;
manual symlinks and sourcing `pi-launchers.zsh` are not required for that path.
...
## Source-only portable install

For an intentionally source-only installation, with Pi already installed separately:
...
- symlinks `pi-catalog`, `pi-gateway`, `pi-omlx-repair`, and `pi-vanilla` into `~/.local/bin`
...
## Source-only setup on another machine
