...
name: archon
description: |
  Use when the user wants to run Archon workflows; create workflows or commands;
  set up or configure Archon; initialize .archon in a repo; or troubleshoot Archon.
  Triggers include: "use/run/ask archon", "archon workflow", "create a workflow",
  "new command", "archon setup", "archon config", and "archon init".
  Archon runs AI workflows in isolated git worktrees for parallel development.
  Not for direct Claude Code work; only for delegating to the Archon CLI.
...
1. **Long-running workflows** — Archon workflows can take a while. If your harness supports background shell tasks (e.g. Claude Code Bash `run_in_background: true`), use that and check progress with the harness task tools. In Pi, there is no Bash `run_in_background` option; either let the command run with a generous timeout, or use `nohup ... > ~/.archon/logs/<name>.log 2>&1 &` and check logs/status with `archon workflow status`.
...
3. **One workflow per shell** — Each workflow blocks its shell unless explicitly backgrounded. Run multiple workflows as separate background tasks.
