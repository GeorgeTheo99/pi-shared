...
Delegate each assessment to a separate sub-agent (Pi's `subagent_parallel` tool with one task per assessment, Codex's subagent spawning, etc.). Each returns structured findings as text. Do NOT output findings to the user yet.
